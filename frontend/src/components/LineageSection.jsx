        ...
        <div className="space-y-2 pb-2">
          ...
          {spirit.previousNames?.length > 0 && (
            <div className="flex items-center gap-1 flex-wrap px-1">
              {spirit.previousNames.map((name, i) => (
                <span key={i} className="text-xs px-1.5 py-0.5 rounded bg-purple-900/30 text-purple-300 border border-purple-800/30">
                  {name}
                </span>
              ))}
              <span className="text-xs text-gray-400">→</span>
              <span className="text-xs px-1.5 py-0.5 rounded bg-amber-900/30 text-amber-300 border border-amber-800/30">
                {spirit.name}
              </span>
            </div>
          )}
          ...
          {spirit.pastLifeMemories?.length > 0 && (
            <div className="px-1 space-y-1">
              <div className="text-xs text-gray-500 font-mono uppercase tracking-wider">Echoes</div>
              {spirit.pastLifeMemories.slice(0, 3).map((mem, i) => (
                <p key={i} className="text-xs text-gray-500 italic leading-tight">"{mem}"</p>
              ))}
            </div>
          )}
          ...
          {loading && (
            <div className="px-1 text-xs text-gray-500 font-mono animate-pulse">
              Fetching lineage from Walrus...
            </div>
          )}

          {error && (
            <div className="px-1 text-xs text-red-400 font-mono">{error}</div>
          )}
          ...
          {lineage && lineage.length === 0 && !loading && (
            <div className="px-1 text-xs text-gray-400 italic">
    ...
    <div className={`relative pl-4 ${!isLast ? 'pb-2' : ''}`}>
      ...
      <div className="bg-gray-800/30 rounded px-2 py-1.5 border border-gray-700/20">
        <div className="flex items-center justify-between mb-0.5">
          ...
          {essence.gameOutcome && (
            <span className={`text-xs font-mono ${essence.gameOutcome.isVictory ? 'text-green-400' : 'text-gray-500'}`}>
